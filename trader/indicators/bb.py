...
class BollingerBands(Indicator):
    ...
    """
    def __init__(self, window, num_std) -> None:
        self.window = window
    ...
    def signal(self, df: pd.DataFrame) -> Signal:
        ...
        # Calculate Bollinger Bands
        df['BB_H'] = df[f'BB_SMA{self.window}'] + (df[f'BB_SMA{self.window}'] * self.num_std)
        df['BB_L'] = df[f'BB_SMA{self.window}'] - (df[f'BB_SMA{self.window}'] * self.num_std)
