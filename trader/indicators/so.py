...
class StochasticOscillation(Indicator):
    ...
    """
    def __init__(self, k_period, d_period, overbought, oversold) -> None:
    ...
    def signal(self, df: pd.DataFrame) -> Signal:
        ...
        # Overbought status
        if df['SO_K%'].iloc[-1] > self.overbought and df['SO_D%'].iloc[-1] > self.overbought and df['SO_K%'].iloc[-1] < df['SO_D%'].iloc[-1]:
            return Signal.SELL
        # Oversold status
        elif df['SO_K%'].iloc[-1] < self.oversold and df['SO_D%'].iloc[-1] < self.oversold and df['SO_K%'].iloc[-1] > df['SO_D%'].iloc[-1]:
            return Signal.BUY
