...
class ResidualStrengthIndex(Indicator):
    ...
    """
    def __init__(self, window, overbought, oversold) -> None:
        self.window = window
    ...
    def signal(self, df: pd.DataFrame) -> Signal:
        ...
        # SMA?
        df['avg_gain'] = df['gain'].rolling(window=self.window, min_periods=self.window).mean()[:self.window+1]
        df['avg_loss'] = df['loss'].rolling(window=self.window, min_periods=self.window).mean()[:self.window+1]
        ...
        # Average Gains
        for i, row in enumerate(df['avg_gain'].iloc[self.window + 1:]):
            j = i + self.window
            k = i + self.window + 1
            df['avg_gain'].iloc[k] = (df['avg_gain'].iloc[j] * (self.window - 1) + df['gain'].iloc[k]) / self.window

        # Average Losses
        for i, row in enumerate(df['avg_loss'].iloc[self.window + 1:]):
            j = i + self.window
            k = i + self.window + 1
            df['avg_loss'].iloc[k] = (df['avg_loss'].iloc[j] * (self.window - 1) + df['loss'].iloc[k]) / self.window
