...
class Trading212(Broker):
    ...
    def __init__(self, apiKey, mode='live'):
        ...
        owned=list(map(lambda s: s['ticker'], self.portfolio))     
        self.tracked_market = [stock for stock in self.market if stock['ticker'] in owned]
    
    def get_tracked_symbols(self):
        return list(map(lambda stock: stock['shortName'], self.tracked_market))

    def get_tracked_instrument(self, symbol):
        return list(filter(lambda stock: stock['shortName'] is symbol, self.tracked_market))
    #
    # REST API methods
    #
    ...
    def __check_response(self, response):
        if response.status_code == 401:
            raise self.TokenException(f'[{response.status_code}] {response.reason}: Bad Token')
        if response.status_code == 403:
            raise self.UnauthorizedException(f'[{response.status_code}] {response.reason}: Scope missing for Token')
        if response.status_code == 408:
            raise self.UnauthorizedException(f'[{response.status_code}] {response.reason}: Timeout')
        if response.status_code == 429:
            raise self.RateLimitException(f'[{response.status_code}] {response.reason}: Limited 1/30s')
    ...
    def __api_post(self, url, json, query):
        data = None
        try:
            response = requests.post(url=url, json=json, params=query, headers=self.headers)
            self.__check_response(response)
            data = response.json()
        except JSONDecodeError as e:
            print('Response could not be serialized')
    ...
    def capital(self, symbol) -> float:
        response = self.get_position(symbol)
        return 0.0
