...
class Stock:
    ...
    def update(self) -> None:
        ...
        if not self.history.empty:
            events = events.loc[~events.index.isin(self.history.index)] # filter out duplicates

        self.history = self.history.append(events)
    ...
    def tail(self) -> pd.DataFrame:
        ...
        return self.history.tail(1).copy(deep=True)
